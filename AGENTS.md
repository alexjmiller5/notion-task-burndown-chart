...
- `metrics.ts` — `calculateDailyMetrics` (per-day avg open-task age, p90 open-task age, rolling 14-day median age-at-completion) and `calculateCompletions` (completion counts per day/week/month bucket, split into backlog completions vs same-day turnaround (effective start = completion day)); also exports `bucketLabel` so the chart can align completion rows with downsampled points.
...
- The completion overlay lives inside `TaskChart`: narrow muted-slate bars per bucket on a hidden right axis (`y2`, scaled so the tallest bar fills ~1/4 of the plot), solid = backlog completions, diagonal-hatched = same-day turnaround (created and completed the same day, the churn the burndown nets to zero). Toggled by the Done chip; excluded from the legend; the tooltip footer always shows "Completed: N · M same-day" when the bucket has completions. There is no created side or separate Flow chart: when new obligations start is what the burndown itself shows
