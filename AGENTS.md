...
- `src/lib/legend.ts` — legend-toggle memory shared by both charts (unit-tested). Charts are destroyed and rebuilt on every prop change, so hidden dataset labels live here: toggles survive range/data/bucket changes and Active↔Flow switches, reset (reseeding `hiddenByDefault`, e.g. "(No Project)") only when the category set changes — except `14d avg`, which persists across every view. The 14d avg is computed inside TaskChart (`rollingAvgTotals` over the full daily series via `avgSource`, summing only legend-visible categories) and recalculates on legend clicks
