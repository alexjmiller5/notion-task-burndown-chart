...
- `RateChart.svelte` — the burndown's rate of change, shown when the chart-mode select is on Rate: added (effective starts that joined the pile) up in the marker red, backlog completions down in the marker green, zero baseline, bucket-aware; tooltip footer shows the net. Same-day churn is deliberately absent here — it lives on the Active chart's cap
