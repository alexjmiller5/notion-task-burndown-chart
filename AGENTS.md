...
- `RateChart.svelte` — the burndown's rate of change, shown when the chart-mode select is on Rate: added (effective starts that joined the pile) up in the marker red, backlog completions down in the marker green, zero baseline, bucket-aware; tooltip footer shows the net. The Same-day chip adds same-day churn as a symmetric hatched pair extending both directions (it is both an add and a completion, so it visibly nets zero); the mirror twin is hidden from legend/tooltip and the legend entry toggles both
